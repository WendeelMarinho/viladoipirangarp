...
## 6. Alterações recentes de idioma

- Mensagens visíveis de **veículo** (`/me`, chat, infobox) passadas para **PT-BR** no recurso `oVehicle`.
- **Painel de comandos admin**: `shortDescription` em `g_commands.lua` em PT-BR.
- **Prefixos de patente** em `g_admin.lua` alinhados a termos PT/EN legíveis.
- **Anticheat**: kick/log quando o serial não está em `adminserials` em PT-BR.
